...
## Introduction
Multinomial logistic regression can be implemented using `mlogit()` from mlogit package and `multinom()` from `nnet` package. We will use the later for this example.

## Example: Predict Contraceptive Method Choice

In this example, we will try to predict the choice of contraceptives preferred by women. We have the eduction, work, religion, number of childers, media exposure and standard of living as variables available in the [cmc data](http://archive.ics.uci.edu/ml/datasets/Contraceptive+Method+Choice). In this example, we will model the conraceptive method used `cmc` as a function of all these variables. 
...
A misclassification error of 49.3% is probably too high. May be it can be improved by improving the model terms or may be the variables are not as good in explaining the contraceptive method used. Either ways, I would encourage the analyst to try other ML approaches.
