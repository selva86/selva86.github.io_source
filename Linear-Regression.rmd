...
A low correlation (-0.2 < x < 0.2) probably suggests that much of variation of the response variable ($Y$) is unexplained by the predictor ($X$), in which case, we should probably look for new variables.
...
where, $SSE$ is the _sum of squared errors_ given by $SSE =  \sum_{i}^{n} \left( y_{i} - \hat{y_{i}} \right) ^{2}$ and $SST =  \sum_{i}^{n} \left( y_{i} - \bar{y_{i}} \right) ^{2}$ is the _sum of squared total_. 
