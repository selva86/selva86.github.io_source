---
title: GGplot2 3-Part Tutorial in R (with examples and code) - Part 1
layout: default
sitemap:
    priority: 1
    changefreq: 'monthly'
 ...
 * [Part 2: Advanced customizations](ggplot2-r-Tutorial-Part2.html), is about more advanced customizations like manipulating legend, annotations, multiplots with faceting and custom layouts
 
 * [Part 3: Other types of Ggplot geoms](ggplot2-r-Tutorial-Part3.html), applies what was learnt in part 1 and 2 to construct other types of ggplots such as bar charts, boxplots etc.

### Part 1: Introduction to Ggplot 
1. [Understanding the ggplot syntax](ggplot2-r-Tutorial-Part1.html#1. Understanding the general ggplot format)
2. [Making a simple scatter plot](ggplot2-r-Tutorial-Part1.html#2. Making a simple scatter plot)
3. [Adjusting the X and Y axis limits](ggplot2-r-Tutorial-Part1.html#3. Adjusting the X and Y axis limits)
4. [Changing the title and axis labels](ggplot2-r-Tutorial-Part1.html#4. Changing the title and axis labels)
5. [Changing the color and size of points, title and axis labels and axis text](ggplot2-r-Tutorial-With-R.html#5. Changing the color and size of points, title and axis labels and axis text)
...
The axis limits can be controlled in 2 ways. 

1. One by deleting the points outside the range. This will change the lines of best fit or smoothing lines as compared to the original data.
...
2. The other method to change the X and Y axis limits is to zoom in to the region of interest _without_ deleting the points. This is done using `coord_cartesian()`. 
...
Now each point is colored based on the `state` it belongs because of `aes(col=state)`. As an added benefit, the legend is added automatically. If needed, it can be removed by setting the `legend.position` to `None` from within a `theme()` function.
```{r}
...
Alright, now let's see how to change the X and Y axis text. This involves two aspects: `breaks` and `labels`. 
...
Let's set the `breaks` for Y axis text and format the X and Y axis labels. I have used 2 methods for formatting labels: using `sprintf()` and using a custom function. Use whichever method is convenient.
```{r}
gg + scale_x_continuous(breaks=seq(0, 0.1, 0.01), labels = sprintf("%1.2f%%", seq(0, 0.1, 0.01))) + scale_y_continuous(breaks=seq(0, 1000000, 200000), labels = function(x){paste0(x/1000, 'K')})
```
...
The aesthetics of components such as plot and axis titles and the axis text can also be customized. Since they are part of the plot's theme, it has to be passed to the `theme()` function and the values need to be set using the `element_text()` function. Below, I have changed the size, color, face and line-height. The axis text can be tilted by changing the `angle`. 
```{r}
gg1 <- gg + theme(plot.title=element_text(size=35, 
                                          face="bold", 
...
* `vjust`, increases the spacing between title (or label) and plot.
* `hjust`, setting it to 0.5 centers the title.
* `family`, sets a new font
...
Finally, instead of changing the theme components individually, we can change the entire theme itself using pre-built themes. The help page `?theme_bw` shows all the available built-in themes.
```{r}
gg1 + theme_bw()
gg1 + theme_classic()
```
...
That's it for the basics. We are now ably positioned to tackle more advanced customizations. In [ggplot tutorial part-2](ggplot2-r-Tutorial-Part2.html), I discuss about advanced customizations on modifying theme components, manipulating legend, annotations, faceting and custom layouts.
