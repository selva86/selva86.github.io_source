---
title: The Complete GGplot2 Tutorial - Part1 | Introduction to Ggplot (with code in R)
layout: default
sitemap:
    priority: 1
    changefreq: 'monthly'
...
# Ggplot2 Tutorial - Part 1: Introduction To Gplot
...
5. [Changing the color and size of points](ggplot2-r-Tutorial-Part1.html#5. Changing the color and size of points)
...
1. __Method 1__: By deleting the points outside the range. This will change the lines of best fit or smoothing lines as compared to the original data.
...
2. __Method 2__: The other method is to change the X and Y axis limits by zooming in to the region of interest _without_ deleting the points. This is done using `coord_cartesian()`. 
...
2. __Step 2: Change the `labels()`__: You can optionally change the `labels` at the axis ticks. `labels` take a vector of the same length as `breaks`.
...
Let's set the `breaks` for Y axis text as well and format the X and Y axis labels. I have used 2 methods for formatting labels: 
  ...
  * __Method 2__: Using a custom user defined function. (Formated 1000's to 1K scale)
...
Finally, instead of changing the theme components individually (which I discuss in detail in [part 2](ggplot2-r-Tutorial-Part2.html)), we can change the entire theme itself using pre-built themes. The help page `?theme_bw` shows all the available built-in themes.
