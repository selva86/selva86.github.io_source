...
An example of _linear in variables_. This will violate the assumption.
...
In this case, there is a definite pattern noticed. So, there is heterscedasticity. Lets check this on a 

```{r, eval=FALSE}
mod <- lm(dist ~ speed, data=cars)  #  linear model
plot(mod)
```
...
The residuals should be normally distributed. This also implies the Y and the X's are also normally distributed. It is an essential assumtion if the maximum likelihood method is used to commute the estimates. 
