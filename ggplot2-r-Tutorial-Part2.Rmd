---
title: "GGplot2 3-Part Tutorial in R (with examples and code) - Part 2"
sitemap:
  changefreq: monthly
 ...
 * [Part 3: The Ggplot2 Geoms - What visualization to use for a given objective?](ggplot2-r-Tutorial-Part3.html), applies what was learnt in part 1 and 2 to construct other types of ggplots such as bar charts, boxplots etc.
...
See answer with 20 and 6 votes respectively on changing legen title and names.
...
```{r}
gg + theme(plot.title=element_text(size=30, 
                                    face="bold", 
...
* `vjust`, controls the spacing between title (or label) and plot.
...
There are 3 ways to change the legend title. The output for all three is exactly the same.
###### Using labs()
```{r}
gg + labs(color="State", size="Density")
```

###### Using scale_aesthetic_vartype()
...
#### Legend Positions
The legend's position is related to the theme. So it can be modified using the `theme()` function. If you want to place the legend inside the plot, you can additionally control the hinge point of the legend using `legend.justification`. 
...
# plot text and label that repels from points and eachother (using ggrepel pkg)
library(ggrepel)
...
```{r}
a  <- theme(plot.title=element_text(size=25, face="bold"), 
